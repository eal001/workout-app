...
class SetViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, SetViewControllerDelegate {
    ...
    func update_maxes_label(){
        if info_segment.selectedSegmentIndex == 0{
            max_label.text = "\((exercise!.max_weight.weight * (Constants.KILOS ? 1 : Constants.K_TO_LB)).round(places: 2)) \(Constants.WEIGHT_UNIT())"
    ...
    @IBAction func max_info(_ sender: Any) {
        ...
        let title : String
        if info_segment.selectedSegmentIndex == 0{
            msg = "\((exercise!.max_weight.weight * (Constants.KILOS ? 1 : Constants.K_TO_LB)).round(places: 2)) \(Constants.WEIGHT_UNIT()) : \(exercise!.max_weight.reps) \(Constants.REP_UNIT)"
            ...
            msg = "\((exercise!.max_volume.weight * (Constants.KILOS ? 1 : Constants.K_TO_LB)).round(places: 2)) \(Constants.WEIGHT_UNIT()) : \(exercise!.max_volume.reps) \(Constants.REP_UNIT)"
